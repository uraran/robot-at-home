...
void usage(void) {
  ...
  fprintf (stderr, "-o<filename>\tOutput filename(default: snap.jpg), tip: you can use stdout as filename. imagedata will then be seperated with the string \"{{{NEXt}}}\".\n");
  fprintf (stderr, "-d<device>\tV4L2 Device(default: /dev/video0)\n");
  fprintf (stderr, "-x<width>\tImage Width(must be supported by device)(>960 activates YUYV capture)\n");
  fprintf (stderr, "-y<height>\tImage Height(must be supported by device)(>720 activates YUYV capture)\n");
  fprintf (stderr, "-c<command>\tCommand to run after each image capture(executed as <command> <output_filename>)\n");
  fprintf (stderr, "-t<integer>\tTake continuous shots with <integer> milliseconds between them (0 for single shot)\n");
  fprintf (stderr, "-p<seconds>\tSeconds to wait before first shot\n");
  fprintf (stderr, "-q<percentage>\tJPEG Quality Compression Level (activates YUYV capture)\n");
